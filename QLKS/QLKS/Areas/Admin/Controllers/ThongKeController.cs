...
*/            return View();
        ...
        private void Tab()
        {
            ...
            var tong = db.HoaDons.Where(t => t.ma_tinh_trang == 2 && t.ngay_tra_phong >= firstDayOfMonth && t.ngay_tra_phong <= lastDayOfMonth).Sum(t => t.tien_phong);
            if (tong != null)
            ...
            ViewBag.so_phieu_dat_phong = db.PhieuDatPhongs.Where(u => u.ma_tinh_trang == 1).Count();
            ViewBag.so_phong_dang_dat = db.Phongs.Where(u => u.ma_tinh_trang == 2).Count();
...
*/        }
        private void Char1(DateTime start, DateTime end)
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                var q = db.HoaDons.Where(t => t.ma_tinh_trang == 2 && t.ngay_tra_phong > f1 && t.ngay_tra_phong < f2).Sum(t => t.tien_phong);
                if (q == null)
                    q = 0;
                tong += (double)q;
                C1sl.Add((Double)q);
        ...
        private void Char2(DateTime end)
        {
            ...
            for (int i = 1; i <= 7; i++)
            {
                ...
                DateTime f2 = f1.AddDays(1);
                var q = db.HoaDons.Where(t => t.PhieuDatPhong.ngay_vao >= f1 && t.PhieuDatPhong.ngay_vao < f2).Count();
                C2sl.Add(q);
        ...
        private void Char3()
        {
            /*var s = db.DichVuDaDats.GroupBy(t => t.DichVu.ten_dv).Select(t => new { ten_dv = t.Key, total = t.Sum(i => i.so_luong) });
